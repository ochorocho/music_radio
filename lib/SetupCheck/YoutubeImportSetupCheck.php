	...
	public function run(): SetupResult {
		...
		if (!$status->available) {
			return match ($status->reason) {
				...
				default => SetupResult::info(
					$this->l10n->t('yt-dlp is not installed, so channels cannot import audio from YouTube. Install it with "occ music_radio:ytdlp:install".'),
				),
			};
		}
		...
		if ($status->outdated) {
			return SetupResult::warning(
				$this->l10n->t('The installed yt-dlp (%1$s) is more than 90 days old. YouTube changes frequently and imports will start failing; update it with "occ music_radio:ytdlp:install --force".', [
					$status->ytDlpVersion ?? '?',
				]),
			);
		}
